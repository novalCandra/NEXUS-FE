...
export default function Button({ children, variant = "primary", size = 'md', icon: Icon, path , type } : TypeButton) {
    ...
    return (
        <Link to={path}>
            <button type={type} className={cn(baseStyle, variants[variant], sizes[size])}>{children} {Icon && <Icon size="20" />}</button>
        </Link>
    )
}
